...
- **Typing-first canvas**: Literata 20/28 on a 64ch measure, cursor
...
- **Rich text**: bold/italic/underline/strike/highlight/inline code
  ...
  ctrl-alt-1..3, Literata SemiBold on the 28px rhythm), quotes, lists,
...
- **The margin**: ctrl-m author notes; **ctrl-shift-d runs an editorial
  ...
  right margin as questions, never replacement text. Dismissed diagnoses
  are never re-raised.
- **Settings**: ~/.config/strop/config.toml — [ai] provider, language
  override, auto_copy_selection, font_size.
