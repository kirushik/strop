...
2. **Card motion → ANIMATE moves, never pop, never during a burst.** Appear 250 ms ease-out; re-pack 200 ms standard staggered 40–60 ms; resolve 150 ms ease-in; demote 240 ms standard. No loop, no spring, no scale/spin. Justified by object constancy (Heer & Robertson) — but keep it at UI scale (200–300 ms), *not* the data-viz ~1 s figure. reduced-motion → cross-fade.
   ...
   *Implementation outcome (2026-07-03, completes it):* re-pack moves now SLIDE (200 ms standard, 40 ms stagger capped ×4; margin-card-dynamics.md §14). The "never during a burst" clause is one mechanical rule: a diff of the packed lane between frames tweens only DISCRETE re-packs; scroll, composer growth, and typing-burst reflow snap 1:1 and clear all motion. Deltas ride in content space, so a scroll during a live slide still moves every card exactly with the text. And the `reduce_motion` config switch landed with it: travel degrades to an equal-duration cross-fade (old-slot ghost out, new-slot card in), never a teleport — per §4 below.
