...
/// dictionaries and book fonts, plus the paper-noise tile). These ship as
/// LOOSE files — never `include_bytes!` — because hyph-ru is LPPL 1.2+ and
/// the URW fonts are AGPL-3.0-with-exception: the mere-aggregation posture
...
/// sibling → `../share/strop/assets/` (Linux FHS) →
/// `../Resources/assets/` (macOS bundle) → the repo's `assets/` (dev builds;
/// the compile-time manifest path) → the per-user data dir. Missing
/// everywhere → `None`; callers degrade honestly and log.
pub fn asset_file(rel: &str) -> Option<PathBuf> {
        ...
        && let Some(dir) = exe.parent()
    {
        ...
        candidates.push(dir.join("../share/strop/assets").join(rel));
